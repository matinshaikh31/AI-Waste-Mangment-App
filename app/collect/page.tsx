...
export default function CollectPage() {
  //To store the task that is report
  const [tasks, setTasks] = useState<CollectionTask[]>([]);
  //use To store loding
  const [loading, setLoading] = useState(true);
  const [hoveredWasteType, setHoveredWasteType] = useState<string | null>(null);
  //for seacrhe and filter
  ...
  useEffect(() => {
    const fetchUserAndTasks = async () => {
      ...
      try {
        ...
        console.error("Err  or fetching user and tasks:", error);
  ...
  //use TO Which task user is Selected
  const [selectedTask, setSelectedTask] = useState<CollectionTask | null>(null);
  //Use To verfiy the selected task with the actual task
  const [verificationImage, setVerificationImage] = useState<string | null>(
    null
  );
  //To Show Veficationstatus during loading
  const [verificationStatus, setVerificationStatus] = useState<
    "idle" | "verifying" | "success" | "failure"
  >("idle");
  //To update Verfication result
  const [verificationResult, setVerificationResult] = useState<{
    wasteTypeMatch: boolean;
    quantityMatch: boolean;
    confidence: number;
  } | null>(null);
  //To hold reward after Complete task
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto text-gray-800">
